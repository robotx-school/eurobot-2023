...
void loop()
{
    ...
    {
        if (LIDAR_MIN_DIST_CIRCLE > LIDAR_TRIGGER_DIST)
        {
    ...
    if (spiTranferEnd)
    {
        ...
        case 8: // Play trick with robot color changing
            ...
            break;
        case 9: // Check lidar and stop robot if needed; we can control lidared drive and not lidared drive with sending 9 packet when pinging robots motors to jump for next step
            frontLidar.getData(frontDist);
            ...
            sendData[7] = backDist;
            if (frontDist < LIDAR_TRIGGER_DIST && !WAITING)
            {
