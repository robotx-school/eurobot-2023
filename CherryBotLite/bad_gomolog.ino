...
int LIDAR_MIN_DIST_CIRCLE = 100000; 
...
void checkDist() {
  if (millis() - distTimer > 80) {
    ...
    if (frontDist < LIDAR_MIN_DIST_CIRCLE){
      frontDist = LIDAR_MIN_DIST_CIRCLE;
    }
    distTimer = millis();
  }
}
...
void loop () {
  ...
  //sendData[3] = stepper4.tick();
   ...
   if (servo_pos[9] <= 55) {
     ...
     servo_speed[9] = 5;
     sendData[8] = servo_pos[9];
     LIDAR_SCANNED_CIRCLE = true; // start circle
   }
   else {
     sendData[8] = servo_pos[9];
  }
    ...
    switch (int_data[0]) {
      ...
      case 9:
        ...
        if (frontDist < 70 && !WAITING){
          int base = abs(stepper1.getTarget()) - abs(stepper1.getCurrent());  
          stepper1.brake();
          stepper2.brake();
          WAITING = true;
          while (frontDist < 70){
