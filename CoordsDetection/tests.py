        ...
        for i in range(0, len(ids)):
            if ids[i] in [140, 134, 137]:
                ...
                cv2.drawFrameAxes(frame, matrix_coefficients,
                                  distortion_coefficients, rvec, tvec, 80)
    print("----")
    return frame
...
if __name__ == '__main__':
    ...
    # np.array([[615.5265089586733, 0.0, 309.3014206158378], [0.0, 616.3145552040991, 234.92170696626567], [0.0, 0.0, 1.0]]) #np.load(calibration_matrix_path)
    k = np.array([[896.128972098295, 0.0, 959.6108625623755], [
                 0.0, 901.0779296900218, 534.4228432183535], [0.0, 0.0, 1.0]])
    # np.array([[0.5834358848094446], [-3.892195415621445], [24.02626662491725], [-55.15598762358913]]) #np.load(distortion_coefficients_path)
    d = np.array([[-0.04048025498479023], [0.061095515746160366],
                 [-0.06836476062818098], [0.024622705491456814]])
